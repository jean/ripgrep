    ...
    pub fn build(&self, dent: DirEntry) -> Option<Subject> {
        ...
        if let Some(ref handle) = self.config.skip {
            match subj.equals(handle) {
                ...
                Err(err) => {
                    message!("{}: {}", subj.path().display(), err);
                    debug!(
                        "ignoring {}: got error: {}",
                        subj.path().display(), err
                    );
                    return None;
                }
            }
        ...
        debug!(
            "ignoring {}: failed to pass subject filter: \
             file type: {:?}, metadata: {:?}",
             subj.path().display(), subj.dent.file_type(), subj.dent.metadata()
        );
        None
    }
